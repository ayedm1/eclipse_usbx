...
/*  FUNCTION                                                 RELEASE      */
/*                                                                        */
/*    _ux_dcd_dwc2_initialize_complete                      PORTABLE C    */
/*                                                           6.0          */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*    controller for the Synopsys DWC2.                                   */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
/*    None                                                                */
...
ULONG                   dwc2_register;
    ...
    _ux_dcd_dwc2_fifo_flush(dcd_dwc2, UX_DCD_DWC2_FLUSH_TX_FIFO, UX_DCD_DWC2_FLUSH_FIFO_ALL);
    _ux_dcd_dwc2_fifo_flush(dcd_dwc2, UX_DCD_DWC2_FLUSH_RX_FIFO, UX_DCD_DWC2_FLUSH_FIFO_ALL);

    /* Clear all pending device interrupts.  */
    _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DAINT, 0xFFFFFFFF);

    /* Reset the IN endpoints.  */
    for (ed_index = 0; ed_index < UX_DCD_DWC2_MAX_ED; ed_index++)
    {

        /* Read the current endpoint control register.  */
        dwc2_register =  _ux_dcd_dwc2_register_read(dcd_dwc2, UX_DCD_DWC2_OTG_DIEPCTL + (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE));

        /* Check if the endpoint is already enabled.  */
        if (dwc2_register & UX_DCD_DWC2_OTG_DIEPCTL_EPENA)
        {

            /* Set the endpoint disabled bit and SNAK.  */
            dwc2_register = UX_DCD_DWC2_OTG_DIEPCTL_SNAK | UX_DCD_DWC2_OTG_DIEPCTL_EPDIS;
        }
        else
        ...
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DIEPCTL + (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), dwc2_register);

        /* Reset the DIEPSIZ register of the endpoint.  */
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DIEPTSIZ + (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), 0);

        /* Reset the interrupt register for the endpoint.  */
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DIEPINT + (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), UX_DCD_DWC2_OTG_DIEPINT_ALL);
    ...
    for (ed_index = 0; ed_index < UX_DCD_DWC2_MAX_ED; ed_index++)
    {
        ...
        /* Read the current endpoint control register.  */
        dwc2_register =  _ux_dcd_dwc2_register_read(dcd_dwc2, UX_DCD_DWC2_OTG_DOEPCTL +
                                                        (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE));

        /* Check if the endpoint is already enabled.  */
        if (dwc2_register & UX_DCD_DWC2_OTG_DOEPCTL_EPENA)
        {

            /* Set the endpoint disabled bit and SNAK.  */
            dwc2_register = UX_DCD_DWC2_OTG_DOEPCTL_SNAK | UX_DCD_DWC2_OTG_DOEPCTL_EPDIS;
        ...
        /* Set the Endpoint register.  */
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DOEPCTL + (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), dwc2_register);
        /* Reset the DIEPSIZ register of the endpoint.  */
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DOEPTSIZ +
                                                        (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), 0);

        /* Reset the interrupt register for the endpoint.  */
        _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DOEPINT +
                                                        (ed_index * UX_DCD_DWC2_ENDPOINT_SPACE_SIZE), UX_DCD_DWC2_OTG_DOEPINT_ALL);
    }

    /* Set the DOEPMSK register.  */
    _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DOEPMSK, UX_DCD_DWC2_OTG_DOEPMSK_XFRCM  |
                                                                   UX_DCD_DWC2_OTG_DOEPMSK_STUPM  |
                                                                   UX_DCD_DWC2_OTG_DOEPMSK_EPDM   |
                                                                   UX_DCD_DWC2_OTG_DOEPMSK_B2BSTUP);

    /* Set the DIEPMSK register.  */
    _ux_dcd_dwc2_register_write(dcd_dwc2, UX_DCD_DWC2_OTG_DIEPMSK, UX_DCD_DWC2_OTG_DIEPMSK_XFRCM |
                                                                   UX_DCD_DWC2_OTG_DIEPMSK_EPDM  |
                                                                   UX_DCD_DWC2_OTG_DIEPMSK_TOM);

    /* Reset the Global IN/OUT NAK. */
    _ux_dcd_dwc2_register_set(dcd_dwc2, UX_DCD_DWC2_OTG_DCTL, UX_DCD_DWC2_OTG_DCTL_CGONAK | UX_DCD_DWC2_OTG_DCTL_CGINAK);

    /* Reset the device address.  */
    _ux_dcd_dwc2_address_set(dcd_dwc2, 0);
    ...
    /* Ready to accept the first packet when connected.  */
    return(UX_SUCCESS);
}
